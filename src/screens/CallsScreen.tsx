...
import CallHandlingService, { VoiceProfile } from '../services/CallHandlingService';
...
import type { CallHandlingMode } from '../types/onboarding';
...
const CALL_HANDLING_MODES: Array<{ id: CallHandlingMode; title: string; description: string; badge?: string }> = [
  {
    id: 'sms_links',
    title: 'SMS Link Follow-Up',
    description: 'Send booking and quote links via SMS when you miss a call. No AI configuration needed.',
    badge: 'Default',
  },
  {
    id: 'ai_receptionist',
    title: 'AI Receptionist',
    description: 'Conversational AI that asks questions and gathers job details during calls.',
    badge: 'Premium',
  },
  {
    id: 'voicemail_only',
    title: 'Voicemail Only',
    description: 'Basic voicemail capture with transcription. Simple and straightforward.',
  },
];
...
export const CallsScreen: React.FC = () => {
  ...
  const [callHandlingMode, setCallHandlingMode] = useState<CallHandlingMode>(
    onboardingData.callHandlingMode || onboardingData.receptionistMode as CallHandlingMode || 'sms_links'
  );
  ...
  useEffect(() => {
    ...
    if (onboardingData.receptionistMode) {
      setCallHandlingMode(onboardingData.receptionistMode as CallHandlingMode);
    }
  ...
  }, [selectedVoice, greeting, followUpQuestions, activeVoiceProfileId, callHandlingMode, ackLibrary]);

  const refreshVoiceProfiles = useCallback(async () => {
    setLoadingProfiles(true);
    try {
      const profiles = await CallHandlingService.listVoiceProfiles();
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      await CallHandlingService.savePreferences({
        ...
        voiceProfileId,
        configured: true,
        mode: callHandlingMode as any, // Map new mode to old field for API compatibility
        ackLibrary,
      });
      ...
      updateOnboardingData({
        ...
        receptionistMode: callHandlingMode as any, // Legacy field
        callHandlingMode: callHandlingMode, // New field
        receptionistAckLibrary: ackLibrary,
      });
      showToast('Call handling settings saved. Flynn will use these settings on the next call.', 'success');
    } catch (error) {
  ...
  const handlePlayGreeting = useCallback(async () => {
    ...
    try {
      ...
      const preview = await CallHandlingService.previewGreeting(trimmedGreeting, selectedVoice, voiceProfileId);
  ...
  const handlePlayQuestion = useCallback(async (index: number) => {
    ...
    try {
      ...
      const preview = await CallHandlingService.previewGreeting(questionText, selectedVoice, voiceProfileId);
  ...
  return (
    <View style={styles.screen}>
      <ScrollView style={styles.container} contentContainerStyle={styles.content}>
        {/* Analytics Card */}
        <FlynnCard style={styles.analyticsCard}>
          <View style={styles.analyticsHeader}>
            <View>
              <Text style={styles.cardTitle}>Call Analytics</Text>
              <Text style={styles.cardHint}>Track performance and conversions</Text>
            </View>
            <TouchableOpacity
              ...
              activeOpacity={0.7}
            >
              <Text style={styles.analyticsButtonText}>View Metrics</Text>
              <FlynnIcon name="chevron-forward" size={18} color={colors.primary} />
            </TouchableOpacity>
          </View>
        </FlynnCard>
        ...
        <FlynnCard style={styles.card}>
          <Text style={styles.cardTitle}>Call Handling Mode</Text>
          <Text style={styles.cardHint}>Choose how Flynn responds when you miss a call.</Text>
          {CALL_HANDLING_MODES.map(option => {
            const isSelected = callHandlingMode === option.id;
            return (
              <TouchableOpacity
                key={option.id}
                style={[styles.modeOption, isSelected && styles.modeOptionSelected]}
                onPress={() => setCallHandlingMode(option.id)}
                activeOpacity={0.85}
              >
                <View style={styles.modeCopy}>
                  <View style={styles.modeTitleRow}>
                    ...
                    {option.badge && (
                      <View style={[styles.modeBadge, option.badge === 'Default' ? styles.modeBadgeDefault : styles.modeBadgePremium]}>
        ...
        <RecordVoiceModal
          ...
          onStopRecording={async () => {
            ...
            try {
              ...
              const profile = await CallHandlingService.createVoiceProfile(
