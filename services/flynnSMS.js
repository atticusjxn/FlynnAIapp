...
// OAuth-based integrations get a link, not a credential prompt
const OAUTH_INTEGRATIONS = new Set(['xero', 'myob', 'quickbooks', 'google_calendar']);
...
- Listen for their real needs and pain points, and suggest the ONE tool that solves each as it comes up: someone who books appointments -> their calendar; someone who invoices or chases payments -> Xero/QuickBooks/MYOB; someone drowning in email -> their inbox; a tradie ordering parts -> their supplier (Reece, Bunnings, etc). Suggest as needed, never dump a list.
...
Set brain_complete=true once you know what they do AND at least one concrete way you can help (a task they need off their plate, or a tool to connect). When complete, name 2 or 3 specific things you'll do for them in their own terms, then offer a quick live proof. Integration slugs you can connect (use only the ones that fit what they told you):
  google_calendar (bookings, scheduling); xero, myob, quickbooks (invoicing, accounting); reece, bunnings, tradelink, nhp, middy, rsea, neco (trade suppliers); amazon (general supplies).`;
...
async function handleIntegrationSetup(message, phone, businessBrain) {
  const client = getLLMClient('compatible');
  const brain = businessBrain || {};
  const pending = brain._pending_integrations || [];
  const connected = brain._connected_integrations || [];

  // Build OAuth links for any OAuth-based integration at the top of the pending list
  const oauthLinks = {};
  for (const slug of pending) {
    if (OAUTH_INTEGRATIONS.has(slug)) {
      oauthLinks[slug] = `${SERVER_URL}/auth/${slug}?phone=${encodeURIComponent(phone)}`;
    }
  }
...
Still to connect: ${pending.join(', ') || 'none'}
Already connected: ${connected.join(', ') || 'none'}
OAuth links available: ${JSON.stringify(oauthLinks)}

Rules:
- Casual, max 2 sentences
- Work through one integration at a time, starting from the top of the pending list
- For OAuth tools (xero, myob, quickbooks): give them the exact link from oauthLinks and say to tap it to connect
- For supplier sites (reece, bunnings, nhp, tradelink, etc.): ask for their trade account email and password on the same line, e.g. "What's your Reece login? Email then password works."
...
- If the user says skip/no/later for an integration, respect it
...
Respond with JSON:
{
  ...
  return {
    ...
    updatedBrain,
    updatedStep: (parsed.onboarding_complete || newPending.length === 0) ? 'active' : 'integrations_pending',
...
async function processMessage({ phone, message, businessBrain, onboardingStep, pendingAction, userIntegrations }) {
  ...
  if (onboardingStep === 'integrations_pending') {
    return handleIntegrationSetup(message, phone, businessBrain);
  }
