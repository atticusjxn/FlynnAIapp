...
async function isUserEntitled(userId) {
  try {
    ...
    console.warn('[Entitlement] check failed (treating as free):', error?.message);
    return false;
  }
}
...
async function computeGoogleSlots(userId, businessHours, { days = 7, durationMins = 60, maxSlots = 5 } = {}) {
  try {
    ...
    console.warn('[Calendar] computeGoogleSlots failed (non-fatal):', error?.status || '', error?.message);
 ...
 * server-side. Apple Calendar writes happen on-device in the app via EventKit,
 * not here.
...
// Phone number provisioning lives in routes/voiceOnboarding.js.
//
// The four routes that used to sit here (/api/telnyx/provision-number, its
// legacy redirect shim, /api/twilio/search-numbers and
// /api/twilio/purchase-number) bought a real Twilio AU number off nothing but
// a valid JWT — no subscription check anywhere — so any account could run up
// rental on demand. Their only remaining caller was the unshipped RN app
// (src/services/TwilioService.ts); the shipped iOS app has always used
// POST /api/voice-onboarding/assign-number, which allocates from the pool
// behind a real entitlement gate.
