/**
 * Daily lead scraper — Puppeteer + system Chrome (no Apify).
 *
 * Strategy: try Google Maps first (richer place cards with website + phone),
 * fall back to Yellow Pages AU when Google Maps yields too few. Rotates
 * trade × city combos by day-of-year so each combo is hit ~once every
 * 2-3 weeks. Writes deduped leads to public.gtm_cold_leads.
 *
 * Run: npm run scrape
 *
...
interface Lead {
  ...
  trade?: string;
  website?: string;
  source: 'puppeteer-google-maps' | 'puppeteer-yellowpages';
}
...
async function scrapeGoogleMaps(browser: Browser, trade: string, city: string): Promise<Lead[]> {
  ...
  return await extractEmailsFromWebsites(browser, places, trade, city, 'puppeteer-google-maps');
}
...
async function scrapeYellowPages(browser: Browser, trade: string, city: string): Promise<Lead[]> {
  ...
  return await extractEmailsFromWebsites(browser, places, trade, city, 'puppeteer-yellowpages');
}
